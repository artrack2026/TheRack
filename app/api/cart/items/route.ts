...
/* POST /api/cart/items
   Body: { productId, sessionId, userId?, existingCartId?, price }
   - Creates cart if needed
   - Checks stock_count > 0
   - Decrements stock by quantity
   - Inserts cart_item row
   - Returns { cartId, cartItemId, expiresAt }
*/
export async function POST(req: NextRequest) {
  if (!isSupabaseConfigured) return NextResponse.json({ error: 'Supabase not configured' }, { status: 503 })

  const { productId, sessionId, userId, existingCartId, price } = await req.json()
  ...
  const { data: product, error: pErr } = await supabase
    .from('products').select('stock_count').eq('id', productId).single()
  ...
  if (cartId) {
    ...
    const { data: newCart, error: cErr } = await supabase
      .from('carts')
      .insert([{ session_id: sessionId, user_id: userId ?? null, expires_at: exp }])
  ...
  const { data: cartItem, error: ciErr } = await supabase
    .from('cart_items')
    .insert([{ cart_id: cartId, product_id: productId, quantity: 1, price_at_add: price }])
    .select('id')
    .single()
